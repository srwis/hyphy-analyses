...
if len(sys.argv) != 3:
    print "\nUsage: MEDSproc.py <resultsFile.csv> <alphaLevel> \n(alphaLevel is the p-value threshold before Bonferroni for 20 tests - recommended 0.01 or 0.05)"
    exit()

file = open(sys.argv[1])
filetext = file.read()
csv_input_file = StringIO.StringIO(filetext)
...
discard = csvReader.next()

MEDSheader = ["Site","AA","altL","alt_p","alt_omega","AltFgNonSyn","AltBgNonSyn","AltSyn","DivL","DivFgNonSyn","DivBgNonSyn","DivSyn"]
...
for row in csvReader:
    ...
    for i in range(20):
        if((p_row[i]<thresh) & (w_row[i] > 0.5)): #only want accelerated changes
                temp_list = [row[0]]
                temp_list.append(aa_list[i])
                #alt details
                temp_list = temp_list + [row[p_indices[i]+j-1] for j in range(2)]
		...
		if (float(row[p_indices[i]+1]) >= 1):
			temp_list.append("Infinity")
                ...
                temp_list = temp_list + [row[p_indices[i]+j-1] for j in range(3,6)]
                #null(div) details
...
discard=csvReader2.next()
FEEDSheader = ["Site","DivL","p","DivFgNonSyn","DivBgNonSyn","DivSyn","NullL","NullBgNonSyn","NullSyn"]
csvprint(FEEDSheader)

for row in csvReader2:
    if((float(row[5])<input_thresh) & (float(row[6])>float(row[8]))): #only want positive selection
        temp_list = [row[0]]
